...
for file in glob.glob(os.path.join(LIST_CHARTS_DIR, "*")):
    os.remove(file)
...
for file_path in txt_files:
    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if len(values) == 9:  
                throughput_data.append(values[:5])  
                memory_data.append([values[0]] + values[5:])  
    ...
    def plot_bar_chart(data, ylabel, chart_name):
        ...
        svg_file_path = os.path.join(LIST_CHARTS_DIR, f"{file_name}_{chart_name}.svg")
        plt.savefig(svg_file_path, format="svg", bbox_inches="tight")
        plt.clf()
        

        pdf_file_path = os.path.join(LIST_CHARTS_DIR, f"{file_name}_{chart_name}.pdf")
    ...
    print(f"Charts saved in {os.path.basename(DATA_DIR)}/{os.path.basename(LIST_CHARTS_DIR)} for {file_name}!")
